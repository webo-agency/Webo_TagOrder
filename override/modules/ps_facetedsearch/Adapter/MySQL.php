...
class MySQL extends AbstractAdapter
{
    ...
    protected function computeOrderByField(array $filterToTableMapping)
    {
        ...
        //IFNULL(p.quantity, 0) <= 0, IFNULL(p.quantity, 0) <= 0 AND FIELD(sa.out_of_stock, 1) DESC, p.position
            
        $selectedTag = (int)Tools::getValue('WEBO_TAGORDER_TAG', Configuration::get('WEBO_TAGORDER_TAG'));
        $sortMaxId = (int)Tools::getValue('WEBO_TAGORDER_SORTMAX', Configuration::get('WEBO_TAGORDER_SORTMAX'));
        if($selectedTag !== ''){
            $tag = new Tag($selectedTag);
            if($tag->id !== null){
                $tagProducts = $tag->getProducts();
                $tagProductsIds = array_column($tagProducts,"id_product");
    
                if(count($tagProductsIds) > 1){
                    $orderField = 'CASE WHEN p.id_product = ' . join(' OR p.id_product = ', $tagProductsIds) . ' THEN p.position + ' . $sortMaxId . ' ELSE p.position END ';
                } else if(count($tagProductsIds) == 1){
                    $orderField = 'CASE WHEN p.id_product = ' . join('',$tagProductsIds) . ' THEN p.position + ' . $sortMaxId . ' ELSE p.position END ';
                }
